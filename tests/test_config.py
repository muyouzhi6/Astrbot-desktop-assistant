...
class TestClientConfig:
    ...
    def test_load_from_dict(self, sample_config_dict: dict, temp_config_file: Path):
        ...
        assert config.appearance.ball_size == sample_config_dict["appearance"]["ball_size"]
...
class TestConfigHelperFunctions:
    ...
    @pytest.mark.unit
    def test_load_config_function(self, temp_config_file: Path, sample_config_dict: dict):
        """测试 load_config 函数"""
        with open(temp_config_file, 'w', encoding='utf-8') as f:
            json.dump(sample_config_dict, f)
    ...
    @pytest.mark.unit
    def test_save_config_function(self, sample_config: ClientConfig, temp_config_file: Path):
        """测试 save_config 函数"""
