...
class TestMessageBridgeFunctionResultExtraction:
    ...
    @pytest.mark.unit
    def test_extract_function_result_normal_text(self, mock_qt_app, sample_config: ClientConfig):
        """测试提取普通文本（不是函数结果）"""
        with patch("desktop_client.bridge.AstrBotApiClient"):
            bridge = MessageBridge(sample_config)
    ...
    @pytest.mark.unit
    def test_extract_function_result_json_with_result(self, mock_qt_app, sample_config: ClientConfig):
        """测试提取 JSON 函数调用结果"""
        with patch("desktop_client.bridge.AstrBotApiClient"):
            bridge = MessageBridge(sample_config)
    ...
    @pytest.mark.unit
    def test_extract_function_result_json_without_result(self, mock_qt_app, sample_config: ClientConfig):
    ...
    @pytest.mark.unit
    def test_extract_function_result_invalid_json(self, mock_qt_app, sample_config: ClientConfig):
        """测试提取无效 JSON"""
        with patch("desktop_client.bridge.AstrBotApiClient"):
            bridge = MessageBridge(sample_config)
    ...
    @pytest.mark.unit
    def test_extract_function_result_empty(self, mock_qt_app, sample_config: ClientConfig):
        """测试提取空内容"""
        with patch("desktop_client.bridge.AstrBotApiClient"):
            bridge = MessageBridge(sample_config)
    ...
    @pytest.mark.unit
    def test_extract_function_result_none(self, mock_qt_app, sample_config: ClientConfig):
    ...
    @pytest.mark.unit
    def test_extract_function_result_whitespace(self, mock_qt_app, sample_config: ClientConfig):
        """测试提取带空白的 JSON"""
        with patch("desktop_client.bridge.AstrBotApiClient"):
            bridge = MessageBridge(sample_config)
...
class TestMessageBridgeSSEEventHandling:
    ...
    @pytest.mark.unit
    def test_handle_reasoning_event_skipped(self, mock_qt_app, sample_config: ClientConfig):
        """测试跳过思维链事件"""
        with patch("desktop_client.bridge.AstrBotApiClient"):
            bridge = MessageBridge(sample_config)
    ...
    @pytest.mark.unit
    def test_handle_empty_non_streaming_skipped(self, mock_qt_app, sample_config: ClientConfig):
        """测试跳过空的非流式消息"""
        with patch("desktop_client.bridge.AstrBotApiClient"):
            bridge = MessageBridge(sample_config)
...
class TestMessageBridgeServerConfig:
    ...
    @pytest.mark.unit
    def test_update_server_config_partial(self, mock_qt_app, sample_config: ClientConfig):
        """测试部分更新服务器配置"""
        with patch("desktop_client.bridge.AstrBotApiClient") as mock_client_cls:
            mock_client = MagicMock()
            mock_client_cls.return_value = mock_client
...
class TestMessageBridgeAsync:
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_connect_server_with_token(self, mock_qt_app, sample_config: ClientConfig):
        """测试使用 Token 连接服务器"""
        with patch("desktop_client.bridge.AstrBotApiClient") as mock_client_cls:
            mock_client = MagicMock()
    ...
    @pytest.mark.asyncio
    @pytest.mark.unit
    async def test_send_input_not_connected(self, mock_qt_app, sample_config: ClientConfig):
        """测试未连接时发送消息"""
        with patch("desktop_client.bridge.AstrBotApiClient") as mock_client_cls:
            mock_client = MagicMock()
            mock_client.is_connected = False
