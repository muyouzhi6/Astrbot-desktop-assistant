...
class ProactiveDialogService(QObject):
    ...
    def _check_user_active(self) -> bool:
        ...
        try:
            ...
            logger.debug(f"用户空闲时间: {idle_seconds}秒，阈值: {self._config.idle_threshold}秒，活跃: {is_active}")
    ...
    def _compress_image(self, image: 'Image.Image') -> Optional['Image.Image']:
        ...
        try:
            ...
            logger.debug(f"图片已压缩: {orig_width}x{orig_height} -> {new_width}x{new_height}")
