# 🖥️ AstrBot 桌面助手客户端

[![GitHub](https://img.shields.io/badge/GitHub-muyouzhi6-blue?logo=github)](https://github.com/muyouzhi6)
[![License](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)
[![Python](https://img.shields.io/badge/Python-3.10+-blue.svg)](https://www.python.org/)
[![PySide6](https://img.shields.io/badge/PySide6-6.5+-green.svg)](https://www.qt.io/qt-for-python)

一个独立运行的桌面悬浮球助手，通过 HTTP API 连接远程 AstrBot 服务器。

> ⚠️ **重要提示**：本项目需要配合 AstrBot 服务端插件一起使用！
> 
> 👉 **配套插件**：[astrbot_plugin_desktop_assistant](https://github.com/muyouzhi6/astrbot_plugin_desktop_assistant)

## 🎯 项目概述

本项目是 AstrBot 桌面助手的**客户端部分**，提供悬浮球界面和用户交互功能。

### 架构说明

```
┌─────────────────────────────────────────────────────────────┐
│                      用户桌面                                │
│  ┌─────────────────────────────────────────────────────┐   │
│  │           AstrBot 桌面助手客户端 (本项目)              │   │
│  │  • 悬浮球界面                                         │   │
│  │  • 对话窗口                                          │   │
│  │  • 截图功能                                          │   │
│  │  • 系统托盘                                          │   │
│  └────────────────────────┬────────────────────────────┘   │
│                           │ HTTP API                        │
│                           ▼                                 │
│  ┌─────────────────────────────────────────────────────┐   │
│  │            AstrBot 服务器 + 插件 (配套项目)           │   │
│  │  • WebChat 平台适配器                                │   │
│  │  • 消息队列管理                                      │   │
│  │  • AI 对话处理                                       │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```

## ✨ 功能特性

- 🔗 **远程连接**: 通过 HTTP API 连接云端或本地 AstrBot 服务器
- 💬 **对话交互**: 支持文本、图片、语音消息
- 🖼️ **截图功能**: 截取屏幕区域并发送给 AI
- 🔔 **系统托盘**: 后台运行，随时唤起
- ⚡ **流式响应**: 支持 SSE 流式消息显示
- 🔒 **安全认证**: JWT Token 认证机制
- 🎨 **主题切换**: 支持亮色/暗色/跟随系统主题
- 🚀 **开机自启**: 支持 Windows 开机自动启动
- 💡 **消息通知**: 悬浮球动态效果提示未读消息

## 📦 快速开始

### 1. 安装 AstrBot 服务端插件

首先，确保你的 AstrBot 服务器已安装配套插件：

```bash
# 在 AstrBot 中安装插件
# 方式1: 通过 AstrBot 插件市场搜索 "desktop_assistant"
# 方式2: 手动克隆到插件目录
git clone https://github.com/muyouzhi6/astrbot_plugin_desktop_assistant.git \
    data/plugins/astrbot_plugin_desktop_assistant
```

### 2. 安装客户端依赖

```bash
# 克隆本项目
git clone https://github.com/muyouzhi6/Astrbot-desktop-assistant.git
cd Astrbot-desktop-assistant

# 安装依赖
pip install -r requirements.txt
```

### 3. 启动客户端

```bash
# 方式1: 作为模块运行
python -m desktop_client

# 方式2: 直接运行
python desktop_client/main.py
```

## 🔧 配置说明

### 命令行参数

```bash
python -m desktop_client [选项]

选项:
  -c, --config PATH     配置文件路径 (默认: ~/.astrbot_client/config.json)
  -s, --server URL      服务器 URL (如 http://localhost:6185)
  -u, --username NAME   登录用户名
```

### 配置文件

配置文件为 JSON 格式，默认位置: `~/.astrbot_client/config.json`

```json
{
  "server": {
    "url": "http://localhost:6185",
    "username": "astrbot",
    "password": "",
    "token": null,
    "auto_reconnect": true,
    "reconnect_interval": 5.0,
    "request_timeout": 30.0
  },
  "appearance": {
    "theme": "auto",
    "opacity": 0.95,
    "always_on_top": true
  },
  "chat_window": {
    "width": 400,
    "height": 600,
    "font_size": 14,
    "show_timestamp": true
  },
  "voice": {
    "enabled": true,
    "auto_send": true,
    "sample_rate": 16000
  },
  "autostart": {
    "enabled": false
  },
  "session_id": null
}
```

### 配置项说明

| 配置项 | 说明 | 默认值 |
|-------|------|--------|
| `server.url` | AstrBot 服务器地址 | `http://localhost:6185` |
| `server.username` | 登录用户名 | `astrbot` |
| `server.password` | 登录密码 | 空 |
| `server.auto_reconnect` | 自动重连 | true |
| `appearance.theme` | 主题 (light/dark/auto) | auto |
| `appearance.opacity` | 窗口透明度 | 0.95 |
| `autostart.enabled` | 开机自启动 | false |

## 🎮 使用指南

### 基本操作

| 操作 | 说明 |
|------|------|
| 单击悬浮球 | 显示/隐藏气泡消息 |
| 双击悬浮球 | 打开对话窗口 |
| 右键悬浮球 | 显示功能菜单 |
| 拖拽悬浮球 | 移动位置 |

### 快捷键

| 快捷键 | 功能 |
|--------|------|
| `Enter` | 发送消息 |
| `Shift + Enter` | 消息换行 |
| `Ctrl + V` | 粘贴图片 |
| `Esc` | 关闭对话窗口 |

### 图片预览

对话窗口和气泡消息中的图片支持：
- 点击缩略图查看大图
- 滚轮缩放
- 拖拽移动
- 复制到剪贴板
- 下载保存

## 📁 目录结构

```
desktop_client/
├── __init__.py          # 包初始化
├── __main__.py          # 支持 python -m 运行
├── main.py              # 应用入口
├── app.py               # 应用主类
├── config.py            # 配置管理
├── api_client.py        # HTTP API 客户端
├── bridge.py            # 消息桥接器
├── requirements.txt     # 依赖列表
├── gui/                 # GUI 组件
│   ├── floating_ball.py # 悬浮球
│   ├── settings_window.py # 设置窗口
│   ├── system_tray.py   # 系统托盘
│   ├── screenshot_selector.py # 截图选择器
│   ├── markdown_utils.py # Markdown 渲染
│   ├── themes.py        # 主题管理
│   └── hotkeys.py       # 快捷键
├── services/            # 后台服务
│   ├── screen_capture.py # 截图服务
│   └── proactive_dialog.py # 主动对话
└── utils/               # 工具模块
    └── autostart.py     # 开机自启动
```

## ❓ 故障排除

### 连接失败

1. 确认 AstrBot 服务器正在运行
2. 检查服务器地址是否正确
3. 确认已安装配套插件
4. 检查用户名密码是否正确
5. 确认防火墙未阻止连接

### SSE 响应异常

1. 检查网络稳定性
2. 增加 `request_timeout` 配置
3. 查看服务器日志

### GUI 无法启动

1. 确认已安装 PySide6: `pip install PySide6`
2. 检查显示驱动

### 开机自启动不工作

1. 确认设置中已启用开机自启动
2. 检查 Windows 启动项设置

## 📄 依赖

| 依赖 | 版本 | 说明 |
|------|------|------|
| PySide6 | ≥6.5.0 | Qt6 GUI 框架 |
| httpx | ≥0.24.0 | 异步 HTTP 客户端 |
| mss | ≥9.0.0 | 截图服务 |
| Pillow | ≥10.0.0 | 图像处理 |
| pyaudio | ≥0.2.13 | 音频录制 (可选) |

## 🔗 相关链接

- **配套插件**: [astrbot_plugin_desktop_assistant](https://github.com/muyouzhi6/astrbot_plugin_desktop_assistant)
- **AstrBot**: [https://github.com/Soulter/AstrBot](https://github.com/Soulter/AstrBot)
- **作者主页**: [https://github.com/muyouzhi6](https://github.com/muyouzhi6)

## 📄 许可证

MIT License

## 🙏 致谢

- [AstrBot](https://github.com/Soulter/AstrBot) - 强大的 AI 聊天机器人框架
- [PySide6](https://www.qt.io/qt-for-python) - 优秀的 Python GUI 框架